...
class CharacterCard:
    ...
    def Ability(self,Task,Player,Game):
        Elements = ["Fire","Earth","Air","Water","Light","Dark"]
        Solved = []
        for card in Player.hand.copy():
            if card.card_type in Elements:
                Solved.append(card)
                Elements.remove(card.card_type)
                if len(Solved)==3:
                    for thing in Solved:
                        Player.hand.remove(thing)
                    break
    ...
    def Wrap_up(self,Player,Game):
        if Player.buffs[4] == 1:
            for i,task in enumerate(Game.task_board):
                if task.points == 3:
                    P = len(Game.task_board)
                    self.Ability(task,Player,Game)
                    if len(Game.task_board)<P:
                        Player.buffs[4] = 0
                        break
        #Draw Phase
