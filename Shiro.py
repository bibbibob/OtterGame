...
class CharacterCard:
    ...
    def Endgame(self,Player,Game):
        pass
    def Count_Air(self,Player,Game):
        Airspells=0
        ...
        while Finish == 0:
            ...
            for Spell in Player.hand.copy():
                if (Spell.card_type == "Light" and Spell.timing == 1) or (Spell.card_type == "Dark" and Spell.timing == 1):
                    ...
                    if Max_Actions(Player,Game):
                        return []
            Airspells = self.Count_Air(Player,Game)
            ...
            for i,task in enumerate(Game.task_board):
                if task.points == 1: #Then attempts to solve 1 point tasks
                    ...
                    if Max_Actions(Player,Game):
                        return []
            if self.Count_Air(Player,Game)<Airspells:
                Player.buffs[4]=0
    ...
    def Wrap_up(self,Player,Game):
        ...
        if len(Player.hand)<max_hand:
            if len(Game.spell_deck) < max_hand-len(Player.hand): #Shuffles if deck is too low
                Game.spell_deck+=Game.discard_pile
                random.shuffle(Game.spell_deck)
                Game.discard_pile = []
            Draw(max_hand-len(Player.hand),Player.hand,Game.spell_deck)
        #Refill Phase
