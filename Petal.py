...
class CharacterCard:
    ...
    def Ability(self,Player,Game):
        ...
        if "Earth" in Player.banned_elements or len(Player.hand)<2:
                return 0
        for Task in Game.task_board:
            if Task.points ==3:
                if (Task.solving_spell_1 in HAND) and (Task.solving_type_1 not in Player.banned_elements) and Task.solving_type_2 == "Earth":
                    ...
                    return 1
                elif (Task.solving_spell_2 in HAND) and (Task.solving_type_2 not in Player.banned_elements) and Task.solving_type_1 == "Earth":
                    print(Player.char.name+" solved: "+str(Task.name))
                    Player.Remove_spell(Task.solving_spell_1,Game)
                    Player.Remove_spell(Player.hand[0].name,Game)
                    Player.solved_tasks.append(Task)
    ...
    def Main(self,Player,Game):
        ...
        while Finish == 0:
            ...
            if Player.buffs[4] == 1:
                Player.buffs[4] -= self.Ability(Player,Game)
