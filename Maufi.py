...
class CharacterCard:
    ...
    def Wrap_up(self, Player, Game):
        if Player.buffs[4] == 1:
            Player.buffs[4] = 0
            print("Maufi used Darude Sandstorm")
            for Target in (Enemy for Enemy in Game.players if Enemy.char.name != Player.char.name):
                if len(Target.hand) > 0:
                    np.random.shuffle(Target.hand)
                    discarded_card = Target.hand.pop(0)  # Remove the first card and save it
                    Game.discard_pile.append(discarded_card)
                    print(f"{Target.char.name} discarded {discarded_card.name}")
                else:
                    print(f"{Target.char.name} is empty handed")
        #Draw Phase
