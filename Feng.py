        ...
        while Finish == 0:
            ...
            for Spell in Player.hand:
                if (Spell.card_type == "Light" and Spell.timing == 2) or (Spell.card_type == "Dark" and Spell.timing == 2):
                    ...
                    if Max_Actions(Player,Game):
                        return []
            if Player.buffs[4]==1 and Finish==1:
                Finish = 0
                Player.buffs[4] = 0
                Target = Choose_enemy(Player,Game)
                if len(Target.hand)>0 and len(Player.hand)>0:
                    print("Feng swapped "+Player.hand[0].name+" for "+Target.char.name+"'s "+Target.hand[0].name)
