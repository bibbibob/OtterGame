...
class CharacterCard:
    ...
    def Count_Water(self,Player,Game):
        Waterspells=0
        for card in Player.hand:
            if card.card_type=="Water":
                Waterspells+=1 
        return Waterspells
        ...
        while Finish == 0:
            ...
            for Spell in Player.hand.copy():
                if (Spell.card_type == "Light" and Spell.timing == 1) or (Spell.card_type == "Dark" and Spell.timing == 1):
                    ...
                    if Max_Actions(Player,Game):
                        return []
            Waterspells=self.Count_Water(Player,Game)
            ...
            for i,task in enumerate(Game.task_board):
                if task.points == 1: #Then attempts to solve 1 point tasks
                    ...
                    if Max_Actions(Player,Game):
                        return []
            if self.Count_Water(Player,Game)<Waterspells and Player.buffs[4]==1:
                Draw(1,Player.hand,Game.spell_deck)
                Player.buffs[4]=0
                Finish=0
