...
class CharacterCard:
    ...
    def Use_Ability(self,Player,Game):
        c = 0
        if Player.buffs[4]!=1:
            return 0
        for Task in Game.task_board:
            if Task.points==3:
                c+=1
        if c>=4:
            print(f"Beirui used her ability to draw {c}!")
            Draw(c,Player.hand,Game.spell_deck)
            Player.buffs[4] = 0
