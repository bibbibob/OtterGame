        ...
        while Finish == 0:
            ...
            for Spell in Player.hand:
                if (Spell.card_type == "Light" and Spell.timing == 2) or (Spell.card_type == "Dark" and Spell.timing == 2):
                    ...
                    if Max_Actions(Player,Game):
                        return []
            if Finish==1 and len(Player.hand)>3 and Player.buffs[4]==1:
                print("Franzi is using his ability!")
                k = len(Player.hand)
                for card in Player.hand.copy():
                    Game.discard_pile.append(card)
                    Player.hand.remove(card)
                Draw(k+2,Player.hand,Game.spell_deck)
                Player.buffs[4] = 0
                Finish = 0
