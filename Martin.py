...
class CharacterCard:
    ...
    def Pregame(self,Player,Game):
        np.random.shuffle(Game.spell_deck)
        c=0
        for card in Game.spell_deck:
            if card.card_type == "Dark" and c<2:
                Player.hand.append(card)
                Game.spell_deck.remove(card)
                c+=1
                print(f"Martin has taken {card.name} from the deck with his ability")
        np.random.shuffle(Game.spell_deck)
